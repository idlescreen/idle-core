...
fn is_trusted_control_peer(pid: u32) -> bool {
    ...
    let target = match std::fs::canonicalize(path) {
        Ok(t) => {
            tracing::info!("D-Bus auth check: canonical path for PID {} is {:?}", pid, t);
            t
        }
        Err(e) => {
            tracing::warn!("D-Bus auth check: failed to canonicalize /proc/{}/exe: {:?}", pid, e);
            return false;
        }
    };
    ...
    if !TRUSTED_CONTROL_PEERS.contains(&name) {
        tracing::warn!("D-Bus auth check: process name {:?} is not in trusted control peers list", name);
        return false;
    }
    let parent = target.parent().and_then(|p| p.to_str()).unwrap_or("");
    let is_ok = parent == "/usr/bin" || parent == "/usr/local/bin" || {
        if let Ok(current_exe) = std::env::current_exe() {
            if let Ok(current_canonical) = std::fs::canonicalize(current_exe) {
                ...
                tracing::info!("D-Bus auth check: comparing parent of target {:?} and current_canonical {:?} -> {}", target.parent(), current_canonical.parent(), match_parent);
                match_parent
            } else {
    ...
    if !is_ok {
        tracing::warn!("D-Bus auth check: path {:?} parent {:?} not trusted", target, parent);
    }
    is_ok
}
